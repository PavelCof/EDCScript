...
//window.onload = function(){}
  ...
  let id$ = function (id){ return document.getElementById(id); }// поиск по id (принимает id)
  ...
  let clickId$= function (id,func){event.preventDefault();  id$(id).addEventListener('click', func); }//привязать EventListener по id
  let clickC$= function (classEl,func){let c= class$(classEl); c.forEach(( item, index) => {c[index].addEventListener('click', func);  }); }//привязать EventListener по классу
    ...
    let getOption$=function (idChildEl,elArray,elId){
        let getfiltresAr =function () {
          ...
          // var opt1=filterArr0$(elId, filtresAr);
           if( typeof (opt)!=='undefined') {filtresAr[elId]=opt[0]; id$(elId).style.backgroundColor='white';errors='';}else{console.log('Err',elId); id$(elId).style.backgroundColor='pink'; errors='Введите верные данные';
           }
            if(elId=='line'){
...
let getOptionSelect$=function (idChildEl,elArray,elId){
  //opts=`<option id='' value=""></option>`;
  let getfiltresAr =function () {
    let vals=this.value;
    let l='';
    if(elArray.length>1){
  ...
  changeId$(elId,getfiltresAr);
  let opts='<option id="" value=""></option>';
  elArray.forEach((item, i) => {
    opts+=`<option id='${idChildEl}_${item[0]}'value="${item[1]}">${item[1]}</option>`;
  }); 
        id$(idChildEl).innerHTML=opts; 
};

// Валидация содержимого формы (получает id формы все поля формы должны иметь собственный id.  обращаемся и красим поля по их id)
  let formValid$ = function (formId){
    console.log(formId);
    let elements = id$(formId).elements;
    let element='';
    for (let i = 0, element; element = elements[i++];) {
        if (element.value === ""){
            console.log("Подкрашены пустые поля формы")
            id$(element.id).style.backgroundColor='pink';
          }else{id$(element.id).style.backgroundColor='white';}
          let vals1 = '';
        if(element.id=='applicant_phone'){ let vals1=(/^[0-9]{3,12}$/).test(element.value); console.log(vals1); if(vals1==false){id$(element.id).style.backgroundColor='pink';}else{id$(element.id).style.backgroundColor='white';}}
        
        if(element.id=='responsible_phone'){ let vals1=(/^[0-9]{3,12}$/).test(element.value); console.log(vals1); if(vals1==false){id$(element.id).style.backgroundColor='pink'; }else{id$(element.id).style.backgroundColor='white';}}

        if(element.id=='responsible_fio'){ let vals1=(/^[А-ЯЁ][а-яё]{2,}([-][А-ЯЁ][а-яё]{2,})?\s[А-ЯЁ][а-яё]{2,}\s[А-ЯЁ][а-яё]{2,}$/).test(element.id); console.log(vals1); if(vals1==false){id$(element.id).style.backgroundColor='pink';}else{id$(element.id).style.backgroundColor='white';}}
    
    }
  };
  //Валидация ФИО и телефона
  let ValElId$ = function (typeEl,ElId){
    if(typeEl=='phone'){ let vals1=(/^[0-9]{3,12}$/).test(vId$(ElId)); }
    if(typeEl=='fio'){ let vals1=(/^[А-ЯЁ][а-яё]{2,}([-][А-ЯЁ][а-яё]{2,})?\s[А-ЯЁ][а-яё]{2,}\s[А-ЯЁ][а-яё]{2,}$/).test(vId$(ElId));}
      console.log(vals1); 
        if(vals1==false){
          id$(ElId).style.backgroundColor='pink'; 
        }else{
          id$(ElId).style.backgroundColor='white';
        }
  }